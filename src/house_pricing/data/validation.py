...
class HousingSchema(BaseModel):
    ...
    Latitude: float = Field(..., ge=32, le=42) # Limites approximatives de la Californie
...
def validate_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        try:
            ...
            errors += 1
            if errors < 5: # On loggue seulement les premières erreurs pour ne pas spammer
                logger.warning(f"⚠️ Ligne {index} invalide : {e}")
    
    total_rows = len(df)
    error_rate = errors / total_rows
    
    logger.info(f"📊 Rapport Qualité : {len(valid_rows)} valides / {errors} invalides ({error_rate:.2%})")

    # Règle du PRD : "Arrêt immédiat si qualité < 95%"
    if error_rate > 0.05:
        raise ValueError(f"❌ QUALITÉ CRITIQUE : Taux d'erreur {error_rate:.2%} supérieur au seuil de 5%. Pipeline stoppé.")
